...
- Gemini APIでは、人格・応答ルール・Discord表示名による会話形式を`systemInstruction`へ送信します。
- Discord履歴と現在の質問は`contents[]`の会話ターンとして送信します。人間・外部Botは`role=user`、このBot自身の過去回答だけは`role=model`とし、連続する同一roleは1つのContentへまとめます。複数人は`<Discord表示名>: <発言>`で識別します。
